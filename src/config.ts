...
export const httpConfig = () =>
  loadGroup("http", {
    ...
    /**
     * 카톡 분할 발송 통수. 템플릿 1통이 200자라 3통이면 최대 600자다.
     * 내용이 모자라면 그만큼만 보낸다 — 800자를 채우려고 늘려 쓰지 않는다.
     * 카카오 쿼터는 발신자/수신자 pair당 일 20건이므로
     * 도메인이 3개(주식·부동산·청약)라 3통씩이면 9건, 상한의 45%다.
     * 4통씩이면 12건이라 재실행 여유가 2회밖에 남지 않는다.
     */
    KAKAO_MAX_MESSAGES: z.coerce.number().int().min(1).max(8).default(3),
  });
